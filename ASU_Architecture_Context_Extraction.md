...
- **实现原理**：由于 Electron 应用的文本缓冲区对外部 Accessibility API 屏蔽，ASU 无法强读。解决方案是开发一个极简的 IDE 伴生插件，该插件在 IDE 内部读取全文，并监听本地端口（如 `localhost:18889`）。当 ASU 探测到当前是 IDE 时，静默向该端口发起请求拉取文本。
...
- **阶段二（攻克 30% 核心场景）**：
  ...
  - 在 ASU 中集成对 `localhost:18889` 端口的智能拉取逻辑。
- **阶段三（完善 10% 边缘场景）**：
