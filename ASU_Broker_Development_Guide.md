...
供 ASU Agent 建立长连接，实现系统状态变化的"无感感知"。（`events_probe.py` 已预留框架，待挂载 NSWorkspace 通知）
...
4. **权限弹窗处理**：当 Broker 第一次尝试截取屏幕或读取浏览器时，macOS 可能会弹出如下提示：
   ...
   **必须点击"好"或在系统设置中予以"允许"**。一旦授权，原生终端就会永久持有这块"免死金牌"。此后，无论 ASU 主程序在什么沙盒里运行，只要通过 HTTP 请求这个原生终端里跑着的 Broker，就能无视沙盒边界！
