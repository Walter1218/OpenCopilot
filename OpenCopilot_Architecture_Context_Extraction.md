# OpenCopilot 全场景智能上下文获取方案 (Architecture Proposal)

> **版本**: v2.0 | **日期**: 2026-05-28 | **状态**: UI组件阶段1-4已完成

## 1. 方案背景与核心痛点
在 macOS 环境下开发全局 AI 悬浮伴生工具（如 OpenCopilot），面临着一个严峻的系统级矛盾：
...
## 2. 核心交互流程设计
1. **唤醒动作**：用户在任何软件界面中，**双击或三击鼠标右键**。
2. **场景嗅探**：OpenCopilot 后台异步探测当前活跃窗口（IDE 插件端口 / AppleScript 前台应用名）。
3. **UI 呈现**：
   - **双击右键** → 快捷悬浮卡片（400×300），显示场景按钮（IDE/浏览器读取）+ 快捷指令。
   - **三击右键** → 任务工作台（520×480），用于设定全局任务背景、独立深度对话。
4. **上下文贯通**：工作台设定的任务自动注入到所有快捷卡片的 AI 请求中。Agent 根据来源（IDE/浏览器/拖拽）和当前任务，动态构建场景化 system prompt。若探针失败，UI 平滑降级提示用户使用"物理拖拽"方式投喂。
...
- **实现原理**：当所有高阶静默探针均失效或不可用时，系统降级。OpenCopilot 卡片提示用户主动选中文本，并将高亮文本**物理拖拽**入悬浮卡片中。卡片采用右上角按钮显式关闭的常驻机制，彻底避免拖拽操作中途因焦点丢失而意外隐藏。
- **优势**：利用系统底层的内存级 MIME 传输（`text/plain`），绕过剪贴板同步延迟，100% 保证操作安全与光标不丢失。
- **实现位置**：`smart_copilot.py` → `AICardWindow.dragEnterEvent()` + `dropEvent()` + 显式 `hide_card()`
