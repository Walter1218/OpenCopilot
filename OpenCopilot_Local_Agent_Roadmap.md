...
> **文档状态**: V2.1  
> **更新日期**: 2026-05-28
> **状态**: P0-P2 阶段（UI 组件、主动感知、多模态、Persona 工作坊）已全面完成  
...
OpenCopilot 当前已经从"悬浮 AI 卡片原型"演进到"具备持久心智与多模态主动感知的 OS 级智能体"。核心链路已经打通：
...
- **可用性**：已经具备实际使用价值，真正实现了跨应用跟随与多模态感知。
- **架构方向**：Agent / Broker / IDE Extension / Context 四层拆分是正确的。
- **产品成熟度**：P0-P2 阶段的核心功能已全部落地，UI 与底层引擎完美闭环。
- **下一阶段重点**：深化 Agent Core (如多 Provider 故障转移)、扩展 IDE 深度上下文。
...
- 监听 `127.0.0.1:18888`。
- 提供 `GET /health` 健康检查。
- 提供 `POST /v1/agent/chat` 对话接口，**已支持 `image_base64` 多模态输入**。
- 支持 SSE 流式输出。
- 支持基于 `session_id` 的内存与 SQLite 双重持久化会话。
- 支持多 Persona：
  - `translate`
  - `code`
  - `polish`
  ...
  - `revision`（文档全文联动修订）
  - `default`
  - **支持通过 UI 角色工坊 (Persona Workshop) 动态编辑与热加载**。
- 支持上下文来源注入：
  - `ide`
  - `browser`
  - `drag`
...
- 双击右键唤出快捷悬浮卡片，**支持基于 AXUIElement 无感读取系统高亮选区**。
- 三击右键唤出 Agent Workspace。
- 支持拖拽文本投喂（`dragEnterEvent` + `dropEvent`）。
- **[新增] 支持一键截取前台窗口并进行多模态视觉 OCR 分析**。
- **[新增] 监听系统级焦点变化（WebSocket），托盘状态随应用切换联动**。
- 支持 IDE 全文读取。
...
**已引入 `ContextWindowManager`**：基于字符预算驱动（默认 ~30K token 输入窗口），按来源差异化裁剪，39/39 测试通过，平均压缩率 69%。

### 3.4 Broker 产品化程度不足 🔶（大部分已解决）

Broker 核心探针能力完整，目前事件推送已经落地：

- ✅ WebSocket 主动事件推送已落地（`events_probe.py` 结合 `server.py`）。
- ✅ LaunchAgent 常驻部署 → Agent + Broker 均已完成。
- ✅ Token 明文泄露 → 已改为脱敏显示。
- ✅ 错误格式不统一 → 已统一为 `{"status":"error","code":"...","message":"..."}`。
- 剩余问题：权限引导仍偏手动，需在系统层级实现友好的引导交互。

### 3.5 IDE Extension 上下文粒度还不够（部分已解决）

当前插件已提供：

- ✅ 当前文件全文（`GET /context`）
- ✅ 当前选区内容 + 范围（`GET /selection`）
- ✅ 回写内容到 IDE（`POST /apply`，支持全文替换和局部替换）
...
| `GET /selection` | 当前选区内容和范围 | ✅ |
| `POST /apply` | 回写内容到 IDE（全文/局部替换） | ✅ |
...
- ✅ 实现 `ws://127.0.0.1:18889/ws/events` 或类似 WebSocket 广播通道。
...
`/selection` 和 `/apply` 已完成，实现了选中代码修改 + 一键回写的核心闭环。

### 3. Broker v2

- [x] `capabilities` 接口。
- [x] 统一错误结构。
- [x] WebSocket 前台应用切换事件。
