...
|------|------|------|
| `text` | 纯文本 | 默认类型 |
| `data_comparison` | 数据对比 | "Q1增长10%，Q2增长15%" |
| `time_series` | 时间序列 | "2024年1月至3月销售额" |
| `process` | 流程步骤 | "第一步：需求分析\n第二步：设计" |
| `person_attributes` | 人物属性 | "姓名：张三，年龄：30岁" |
| `list` | 列表 | "- 优点1\n- 优点2" |
| `problem_solution` | 问题-解决方案 | "问题：用户流失\n解决方案：优化体验" |
| `pros_cons` | 优缺点对比 | "优点：速度快\n缺点：成本高" |
| `feature_list` | 功能特点 | "核心功能：数据分析、智能推荐" |
| `case_study` | 案例分析 | "案例：某公司通过方案提升30%效率" |
| `definition` | 定义/概念 | "人工智能是指由人工制造的智能系统" |
| `summary` | 总结/结论 | "总结：本次项目取得显著成果" |
| `quote` | 引用/名言 | "乔布斯曾说过：简洁是最终的复杂" |
| `statistics` | 统计数据 | "据统计，80%的用户对产品满意" |
| `comparison` | 通用对比 | "方案A vs 方案B" |
| `organization` | 组织架构 | "CEO → CTO → 开发部" |
| `timeline` | 时间线 | "里程碑：2024年1月启动，6月完成" |
| `argument` | 论点/论据 | "论点：远程办公更高效" |

**检测模式**：
- **正则模式**（默认）：快速、无依赖，适合结构化内容
- **LLM 辅助模式**：更准确，适合复杂语义内容，需提供 `llm_classify_func`
