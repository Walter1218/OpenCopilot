...
1. **三引擎动态热切架构 (Tri-Engine Architecture)**
   ...
   - 🤖 **本地智能体 (OpenClaw Agent)**：原生对接 OpenClaw CLI，自动嗅探本机加载的智能体（如 main, ginger-eng），在悬浮卡片中直接指挥 Agent 执行复杂工具链任务。
...
3. **双图层解耦与边缘碰撞算法 (Layer Decoupling & Collision Detection)**
...
4. **极低资源占用的系统级监听 (Low-Footprint System Hooks)**
   - 使用 pynput 和 PyQt6 QCursor.pos() 相结合的方案，精准捕获 Retina 高 DPI 屏幕逻辑坐标。
