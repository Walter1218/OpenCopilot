...
> **版本**：v2.7 | **日期**：2026-06-03 | **状态**：管线全计时诊断完成（非LLM部分<5ms），API Gateway async死锁修复，端到端全链路测试通过
...
2. **双引擎动态热切架构 (Dual-Engine Architecture)**
   ...
   - ☁️ **云端 LLM (MiMo/MiniMax)**：MiMo 按量计费性价比高，MiniMax 开箱即用。
...
### 11. MCP Server 实现 (2026-06-02)
- **功能**：实现 Model Context Protocol 服务器，让外部工具（Claude Desktop、Cursor）能够查询 OpenCopilot 的能力
- **暴露工具**：
  1. `query_knowledge_graph` - 知识图谱查询（482实体，329关系）
  2. `search_memory` - 伴生记忆搜索（L2+L3）
  3. `get_broker_status` - Broker系统状态获取
  4. `search_conversation` - 历史对话搜索
  5. `get_system_info` - 系统信息获取
- **通信方式**：JSON-RPC over stdio（安全性优先，不开放网络端口）
- **配置文件**：`.mcp.json`（Claude Code）、`~/.config/claude/claude_desktop_config.json`（Claude Desktop）
- **使用方式**：
  - Claude Desktop：重启后即可使用
  - Claude Code：`claude mcp add opencopilot python tools/mcp_server.py`
  - 命令行：`python tools/mcp_server.py`
- **测试验证**：15个测试全部通过
- **文档**：[MCP_Usage_Guide.md](MCP_Usage_Guide.md)

### 12. Provider 故障转移功能 (2026-06-02)
- **功能**：LLM Provider 自动故障转移，保障服务可用性
- **核心特性**：
  - 多 Provider 自动切换（云端 → 本地 → Agent）
  - 故障检测与自动恢复
  - 失败计数与阈值控制
  - 健康检查机制
- **API端点**：
...
- **配置支持**：通过 `config.json` 配置故障转移参数
- **测试验证**：13个测试全部通过
- **文档**：[MCP_Usage_Guide.md](MCP_Usage_Guide.md)

### 13. 跨文件符号分析功能 (2026-06-02)
- **功能**：项目级别的符号索引和跨文件分析
- **核心能力**：
  - 项目符号索引构建
  - 跨文件符号查找
  ...
  - 项目符号统计
- **API端点**：
  - `POST /api/code/project/index` - 索引项目
  - `POST /api/code/project/search` - 在项目中搜索符号
  - `POST /api/code/project/references` - 在项目中查找符号引用
  - `GET /api/code/project/symbols` - 获取项目所有符号
  - `GET /api/code/project/statistics` - 获取项目符号统计信息
- **性能优化**：支持增量索引，文件哈希检测变化
- **测试验证**：24个测试全部通过
- **文档**：[MCP_Usage_Guide.md](MCP_Usage_Guide.md)

### 14. 全局测试修复 (2026-06-02)
- **功能**：修复之前失败的3个测试，实现100%测试通过率
- **修复内容**：
  1. `test_concurrent_execution` - 并发执行状态检查问题
  2. `test_context_window_manager_env_override` - 环境变量默认值不匹配
  3. `test_minimax_provider_init` - 模型版本不匹配
- **修复方案**：
  - 添加 `_execute_direct` 方法支持并发执行
  - 更新测试期望值匹配实际默认值
- **测试结果**：934个测试全部通过，100%通过率
- **修改文件**：`tool_system/executor.py`、`tests/unit/test_badcase_edge_cases.py`、`tests/unit/test_core_functionality.py`
...
| 多引擎 AI 后端 (MiMo + MiniMax + Ollama) + 设置面板 | LLM 接入 | ✅ |
...
| 多 Provider 故障转移 (MiMo → MiniMax → 本地) | 稳定性 | ✅ |
...
- 👉 [MCP 使用指南](MCP_Usage_Guide.md) - MCP Server 配置和使用
