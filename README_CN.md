...
| **Studio** | 独立窗口壳、文本/幻灯片加载、导出/全屏钩子、`/api/studio/*` 专用路由已存在 | 缩略图导航、可视化预览编辑、可分离对话、统一撤销还未完整接入桌面主链路 |
| **Workspace** | Sidebar + 5 面板外壳、设置入口、刷新钩子已落地 | Task / Chat / Files / Memory 的业务层仍以占位和基础框架为主 |
...
当前代码已经交付了一层 **Studio 基础骨架**：

- `NavigationManager` 管理的独立 `StudioWindowV5`
- `Source / Outline / Preview / 底部 AI 区` 四区域窗口壳
- 文本与 slides 加载钩子、导出 PPT / 全屏预览钩子
- 缩略图渲染、差异预览、全屏预览、Studio 会话状态等 `/api/studio/*` 后端路由

下面这些仍然属于**设计目标 / 迭代中能力**，还没有完整打通到桌面端主交互：

- 缩略图拖拽排序
- WYSIWYG 预览与 Click-to-Edit
- 可分离 AI 对话
- 统一撤销栈
- 完整的差异确认工作流
- 质量徽章与覆盖率热力图
