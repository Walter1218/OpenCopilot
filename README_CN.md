...
| 面板 | 功能 | 说明 |
|------|------|------|
| **Task** | 任务定义与管理 | 任务详情 + 历史 + 模板加载 + 剪贴板/最近文件导入 |
| **Chat** | 会话列表 + 对话 | 多会话切换 + 持久化 |
| **Files** | 最近文件 + 拖放区 | 文件预览 + 导入任务 + 发送 Studio + 复制路径 |
| **Memory** | 知识与上下文 | 知识图谱 + 翻译记忆 + 术语库摘要 |
| **Settings** | 引擎 / 主题 / 快捷键 / 角色 | 统一设置入口 + 当前配置摘要 |
...
| **Smart Copilot** | 3-Tab 主壳、拖拽共享、通过 `V5AgentWorker` 走统一运行时路由 | 更完整的 Markdown 呈现、命令面板、上下文提示仍可继续打磨 |
...
| **Workspace** | Sidebar + 5 面板已贯通；`Workspace Chat` 已接真实 AI，Task/Files/Memory/Settings 作为工作流与上下文面板可用，Files 支持筛选/预览/发送 Studio/发送聊天/复制内容与路径，Memory 支持概览卡片/详情/注入任务与聊天，Settings 支持摘要/导出/复制/重置外观 | 更深层文件管理和知识浏览器仍可继续增强 |
| **统一设置** | Engine / Appearance / Shortcuts / Advanced 已统一到一个弹窗，并通过 bridge 持久化 | 更细的校验、更多配置项摘要和覆盖范围仍在完善 |

### 当前验收基线

当前与 `V5 UI` 直接相关的主 AI 链路已经完成生产级功能验收：

- 已接真实 AI：`Work Tab` 主操作、`Chat Tab` 核心对话、`Workspace Chat`、`Studio Tab`、`Studio Window` 页内共创
- 未接真实 AI 的仍主要是工作流或尚未落地的设计项：`Workspace Files/Memory/Settings/Task`、`Work More`、`Skill Panel`、`Cmd+K`、右键推荐技能菜单
...
详见：`docs/CURRENT_UI_AI_ACCEPTANCE_20260609.md`

### 当前架构亮点

这次当前实现里最值得对外强调的，不只是“又多接了一个智能体入口”，而是 **UI 与智能体执行层的关系已经重构了**：

- **固定一套 `V5 UI`**：`Work / Chat / Workspace Chat / Studio` 继续复用同一套交互入口，不因切换自研或第三方智能体而分叉出两套界面
- **统一运行时入口**：UI 不再直接依赖具体智能体实现，而是统一经由 `V5AgentWorker` 发起任务
- **统一 API 协议化**：第三方智能体统一通过 `/vnext/context/snapshots`、`/vnext/tasks`、`/vnext/tasks/{id}/events` 接入，任务、事件、结果都走标准协议
- **Gateway / Adapter 解耦差异**：provider 差异被下沉到 `Agent Gateway` 与 `Provider Adapter`，不再把第三方协议细节上浮到 UI
- **配置化切换能力已落地**：当前已支持在 `Settings -> Engine -> Agent Runtime` 中切换 `Self Agent` 与 `Third-Party Agent`
- **当前第三方智能体已支持 `Hermes Local`**：并且 `Agent Model` 已经会真实透传到 `/vnext/tasks` 与 `Hermes` run payload

用更直接的话说，当前不是“UI 勉强兼容了 Hermes”，而是已经形成了：

`固定 UI -> 统一 Runtime -> 统一 API 协议 -> Gateway / Adapter 承接智能体差异`

这条链路的价值是：

- 后续继续增强自研智能体，不需要重写 UI 主入口
- 后续新增第三方智能体时，主要改动会集中在 Runtime / Gateway / Adapter，而不是再次把 UI 改乱
- UI、协议、provider 能够分层演进，整体可维护性和可扩展性都明显更强
...
- `Work`、`Chat`、`Studio/PPT` 已统一通过 `gui/v5/agent_worker.py` 进入运行时执行层
- `V5AgentWorker` 现在按 `agent_runtime` 动态路由：默认走 `/vnext/* -> hermes_local`，也支持 `self_agent`、capability 覆盖和 fallback policy
- 非 AI UI 操作主要通过 `gui/v5/bridge.py` 直连本地 Python 模块
- 更完整的启动与运行时说明以 `docs/STARTUP_GUIDE.md` 为准
...
```bash
# 终端 1：Broker 特权代理 (:18889) - 选区 / 活动文档 / 回写必需
bash start_broker.sh

# 终端 2：UI
bash scripts/start_ui.sh

# 可选：如果你需要稳定抓取 API 日志，可手动预热 vnext API
...
- `Agent Model` 配置当前第三方模型，现已真实透传到 `/vnext/tasks` 与 `Hermes` run payload
- `Capability Routes` 控制不同能力是否走第三方
- `Fallback Policy` 控制第三方失败后的自动回退

当前通过配置切换的第三方智能体仍只支持 `Hermes Local`；自研智能体与第三方智能体可通过同一套设置切换。如果要新增其他第三方 provider，需要同时补运行时 adapter、UI preset、测试与文档。详细步骤见 `docs/STARTUP_GUIDE.md` 与 `docs/AGENT_RUNTIME_TARGET_ARCHITECTURE.md`。

从架构上看，这一能力的关键不只是“支持 Hermes”，而是：

- UI 层只关心统一任务入口，不直接持有 provider 细节
- 第三方智能体通过统一协议接入，而不是把第三方 SDK 直接塞进 UI
- 因此当前已经具备把更多第三方智能体继续挂到同一套 UI/Runtime 体系下的基础
...
├── api/                          # API 网关 / vnext API（8010 优先，8000 回退）
│   ├── app.py                    #   路由工厂（统一注册中心）
│   └── routers/                  #   16+ 独立路由模块
...
├── asu_custom_agent.py           # 旧兼容 Agent 服务入口 (:18888)，当前 v5 主链路不要求单独启动
...
├── smart_copilot_api.py          # API 入口（8010 优先，8000 回退）
└── pyproject.toml
```
