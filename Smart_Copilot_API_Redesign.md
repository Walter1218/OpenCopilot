# Smart Copilot 能力平台 API 设计方案

> **文档状态**: 已实现（v2.0）
> **更新日期**: 2026-05-30
> **实现文件**: `smart_copilot_platform.py`
> **使用指南**: `Smart_Copilot_API_Guide.md`

本文档描述了 Smart Copilot 能力平台的 API 设计方案。该方案已从"功能列表"升级为"能力平台"，核心理念是**上下文管理 + 动作执行 + 事件系统**。
...
Smart Copilot 能力平台已实现以下核心设计：
1. **能力导向**：按"能做什么"而非"调用什么"设计
2. **上下文优先**：自动获取 IDE、浏览器、剪贴板等上下文
3. **事件驱动**：WebSocket 实时事件系统
4. **可扩展**：支持插件式扩展新能力

与传统的"功能列表" API 相比，能力平台提供了更灵活、更强大的扩展能力。

## 相关文档

- `Smart_Copilot_API_Guide.md` - 能力平台 API 使用指南
- `smart_copilot_platform.py` - 能力平台实现代码
