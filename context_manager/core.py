...
class ConfigManager:
    ...
    def get_context_budget(self) -> ContextBudget:
        """获取上下文预算配置"""
        budget_config = self._config.get("context_budget", {})
        
        return ContextBudget(
            max_input_chars=budget_config.get("max_input_chars", 120000),
            reserve_output_chars=budget_config.get("reserve_output_chars", 30000),
            recent_turns=budget_config.get("recent_turns", 12),
            max_history_msg_chars=budget_config.get("max_history_msg_chars", 8000)
        )
