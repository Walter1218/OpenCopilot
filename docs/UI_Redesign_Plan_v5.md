...
| **Work Tab** | Context Header Bar（发光圆点+宿主icon）、Selection Preview（等宽字体+蓝色竖线）、胶囊式 Context Strip（5数据源）、卡片式 Primary Actions（三色 icon+描述）、Streaming Result Area（header+结构化内容+confidence bar）、Action Bar（快捷键提示+icon 按钮）已全部实现，含 7 个新埋点事件 + LLM 链路追踪集成 | 接入真实 AI 流式渲染、confidence 计算逻辑 |
...
- `Work Tab`：`V5_WORK_CONTEXT_LOADED`、`V5_WORK_MORE_SHOW`、`V5_WORK_COPY_DONE`、`V5_WORK_APPLY_DONE`、`V5_WORK_EXPORT_STUDIO`、`V5_WORK_PREVIEW_SHOW`、`V5_WORK_CARD_HOVER`、`V5_WORK_STREAM_START`、`V5_WORK_STREAM_DONE`、`V5_WORK_CONFIDENCE`、`V5_WORK_HEADER_REFRESH`、`V5_WORK_HEADER_CLEAR`，以及 LLM 链路追踪（`V5_WORK_LLM_START/DONE/ERROR`）
...
├─ Result Area (streaming state) ──────────────────────────────┤
│  📖 Explain Result                     ● Streaming...        │
│  ─────────────────────────────────────────────────────────    │
│  Function: process_selection                                 │
│  This async function takes a text selection and...           │
│  ┌─ Key Operations: ──────────────────────────────────┐      │
│  │ • 1. Parse selection boundaries and validate input  │      │
│  │ • 2. Call analyze_context() for structural analysis │      │
│  │ • 3. Build and return structured dict with findings │      │
│  └─────────────────────────────────────────────────────┘      │
│  The function follows a pipeline pattern...▌                   │
│  ─────────────────────────────────────────────────────────    │
│  Confidence: ████████████████░░░ 87%                          │
├─ Action Bar ─────────────────────────────────────────────────┤
...
**为什么这么设计：**
- **Context Header Bar**：顶部状态栏以发光圆点指示在线状态，竖线分隔来源类型、字数、宿主应用 icon（如 `[VS] VS Code`），右侧刷新/关闭按钮。让用户一眼确认 AI 的"视野范围"
- **Selection Preview**：选中内容以等宽字体 + 语法高亮预览，左侧蓝色竖线标识，右上角 `selected text` 标签。让用户在操作前确认选区正确
- **Context Strip**（数据源切换）：5 个胶囊按钮，active 态带 icon 字母 + 发光圆点，让用户清楚知道 AI 正在"看"什么。默认选中"Selection"，可切换到活动文档、浏览器内容、剪贴板、文件
- **Primary Action Cards**：从扁平按钮升级为三列卡片式——每张卡有 icon + 名称 + 一句话描述，三色区分（蓝 Explain / 绿 Fix / 黄 Polish）。Explain/Fix/Polish 是最常用的三个操作（占 80% 使用量），卡片式视觉上一眼就能定位
- **Secondary Actions**：保持紧凑行，icon + 名称，灰色描边，低频操作不抢占注意力
- **Streaming Result Area**：结构化结果面板，header 显示 action 类型 + streaming 状态灯（绿色脉冲），body 包含函数名、说明、Key Operations 列表，底部 confidence bar 可视化 AI 置信度
- **Action Bar**：左侧显示快捷键提示（如 `Ctrl+Shift+E to explain`），右侧 Copy/Export 带 icon，Apply 按钮绿色填充突出主操作
