# OpenCopilot v5.0 改版方案

> 版本 v5.0 | 2026-06-05 | 全面交互升级 · 架构精简 · 设计体系建立
...
- **三击右键** → Agent Workspace（当前 520×480，深度工作台）
...
| **渐进式展示** | 简单开始，高级功能按需展开 | Studio 默认简单输入 → 高级 4-Panel 编辑器按需展开 |
...
| Tab 5 CoCreation | → **Tab 3 Studio** | 保留入口，打开 4-Panel 工作台 |
...
**入口逻辑：**
- 点击「打开 Studio」→ 直接打开 CoCreationWindow（4-Panel 工作台）
- **有文本** → 直接加载到 Source Panel
- **无文本** → 弹出输入对话框（复用现有 `_show_cocreation_input_dialog()` 逻辑）

**为什么合并 Tab 3 + Tab 5：** 两个 PPT 入口是历史遗留问题——Tab 3 是"PPT 助手"（三个按钮的启动器），Tab 5 是"CoCreation"（另一个启动器）。用户测试中 100% 的人会困惑。统一为一个 Studio 入口，内部直接打开 4-Panel 编辑器。
...
## 六、模块四：PPT 共创工作台（4-Panel Workbench）
...
| **Deprecate PPTPreviewDialog** | 统一到 CoCreationDialog，移除旧入口 | 消除重复入口 |
...
- Studio 窗口是 Smart Copilot 的子窗口，**关闭 Smart Copilot 不关闭 Studio**（Studio 已脱离为独立工作台）
- Settings 弹窗是模态的，**同一时间只允许一个实例**（无论从哪个入口打开）
- 点击其他区域（`_on_global_click`）时，Smart Copilot 自动隐藏（fade out 200ms），Workspace 和 Studio 保持可见
...
└─────────────────────────────────────────────────────┘
      ...
      有文本(>0字符)              无文本(空)
            │                          │
            ▼                          ▼
  _open_cocreation_window()     弹出输入对话框
            │                   (500×400, QTextEdit)
            │                          │
            │                   用户输入/粘贴内容
            │                   或从 .txt/.md 导入
...
**关键交互细节：**
- `_open_cocreation_window()` 先检查 `_cocreation_window` 是否已存在且可见 → 是则直接 `raise_() + activateWindow()`，不重复创建
- 如果窗口已关闭但对象仍在 → `close() + deleteLater()` 清理后重新创建
...
Step 1: Broker 无感划词
  ...
  ↓
  self.current_text = text  (存储到 AICardWindow 实例)
...
Step 2: 用户切换到 Studio Tab
  ...
  ↓
  自动调用 _open_cocreation_window()

Step 3: 创建 CoCreationWindow
  CoCreationWindow(parent=self) → show()
  ↓
...
Step 2: 用户点击「打开 Studio」按钮
  _launch_ppt_cocreation() 或 _open_cocreation_window() 检测到无文本
  ↓
  弹出输入对话框：
  ┌─ 📝 输入 PPT 内容 (500×400) ──────────────────┐
  │                                                 │
...
```
前置条件：用户之前打开过 Studio 并编辑了幻灯片，关闭了 Smart Copilot 但 CoCreationWindow 仍存在

Step 1: 用户再次双击右键 → Smart Copilot 重新出现
Step 2: 切换到 Studio Tab
  _on_tab_changed(2) 检测逻辑：

  if _cocreation_window exists and isVisible():
    状态: "✅ 共创工作台已打开，切换回去即可继续编辑"
    → 用户只需点击 CoCreationWindow 即可继续

  elif _cocreation_window exists and has slides_data:
    slides_count = len(slides_data)
    状态: "上次编辑：N 页幻灯片 — 点击按钮继续编辑"
    → 用户点击「打开 Studio」→ _open_cocreation_window()
...
| Studio | Smart Copilot Tab 3 | 点击「打开 Studio」 | CoCreationWindow |
...
> **文档版本**：v5.0  
> **更新日期**：2026-06-05  
> **设计稿参考**：smart-copilot-redesign.canvas / agent-workspace-redesign.canvas / settings-skills-redesign.canvas / ppt-cocreation-wireframe.canvas
