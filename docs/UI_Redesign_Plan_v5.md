...
> 版本 v5.0 | 2026-06-02 | 方案对照当前实现 · 去除过时描述 · 明确已落地/待接入边界
...
| **NavigationManager** | 已成为 v5 窗口生命周期中枢 | 仍需继续收敛旧版兼容路径 |
| **Work Tab** | 上下文来源切换、按钮分层、Agent 流式调用已接通 | 更细的状态提示、更多上下文动作 |
| **Chat Tab** | 多会话基础 UI、流式输出、停止生成、上下文注入已落地 | 更完整的历史管理和引用体系 |
| **Studio Window** | 独立窗口壳、Source/Outline/Preview/底部 AI 区、导出/全屏钩子、专用 API 已存在 | 缩略图导航、WYSIWYG 编辑、可分离对话、统一撤销仍未完整接线 |
| **Workspace 2.0** | Sidebar + 5 Panel 窗口壳、刷新钩子、设置入口已落地 | Task / Chat / Files / Memory 的业务逻辑大多仍是占位实现 |
| **Unified Settings** | Engine / Appearance / Shortcuts / Advanced 四分区已落地 | 更全面的配置摘要和数据校验 |
...
也就是说，v5 已经进入主交互路径，但仍处于“主壳已上线、部分业务未补完”的阶段。
...
**当前代码入口逻辑：**
- 点击 Studio 入口 → `NavigationManager.open_studio()` → 打开独立 `StudioWindowV5`
- **有文本** → 直接加载到 Source Panel
- **有 slides** → 直接加载到 Outline/统计信息
- **无文本** → 当前保持在 Studio 窗口壳中等待输入，旧版独立共创入口与输入对话框逻辑已退出 v5 主路径
...
> **当前代码状态说明**：5 Panel 的窗口骨架和侧边栏切换已完成，但 Task / Chat / Files / Memory 中的大部分业务仍以占位文本、刷新钩子和基础 UI 为主，文中以下内容更多代表目标态而非全部已完工能力。
...
> **当前代码状态说明**：当前桌面端已实现 4-Panel 窗口壳与文本/slides 加载，但 `Outline` 与 `Preview` 仍是占位式展示，底部 AI 区也还未完整接通自然语言编辑链路。
