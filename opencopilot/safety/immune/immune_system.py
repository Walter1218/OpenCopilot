    ...
    async def _handle_violations(
        ...
        if most_severe.action_taken == ViolationAction.BLOCK.value:
            ...
            if self._human_intervention_handler:
                ...
                return ImmuneResponse(
                    allowed=False,  # 安全策略：无审批处理器时拒绝
                    violations=sorted_violations,
                    message=f"操作被安全策略拦截（需人工审批但审批处理器未注册）：{most_severe.rule_name}",
                    suggestions=[f"请联系管理员配置人工审批处理器，或修改规则 '{most_severe.rule_name}' 的动作为 WARN/LOG"] + result.suggestions
                )
