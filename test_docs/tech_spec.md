...
此处描述的是历史技术基线：早期版本通过 `asu_custom_agent.py` 在 `18888` 端口提供独立 HTTP 服务。当前主链路已经收口为 `V5AgentWorker -> Agent Runtime -> /vnext/*`，不再以 `18888` 作为系统级 UI 的默认前提。
...
```
UI (PyQt6) ←→ V5AgentWorker / Agent Runtime ←→ /vnext/* / Provider
    ↓                      ↓
