...
class TranslateEvaluator:
    ...
    def _evaluate_faithfulness(self, input_text: str, output_text: str) -> EvaluationResult:
        ...
        # 检查关键信息保留
        # 提取人名、地名、专有名词
        proper_nouns = re.findall(r'[A-Z][a-z]+(?:\s[A-Z][a-z]+)*', input_text)
        for noun in proper_nouns:
            if noun not in output_text and noun.lower() not in output_text.lower():
                # 可能是翻译了，检查中文
                pass
    ...
    def _evaluate_terminology(self, input_text: str, output_text: str) -> EvaluationResult:
        ...
        # 检查专业术语是否统一
        # 提取可能的术语（大写开头的英文词组）
        terms = re.findall(r'[A-Z][a-zA-Z]+(?:\s[A-Z][a-zA-Z]+)*', input_text)
        term_counts = {}
        for term in terms:
